...
def f1(grid_string):
    ...
    grid = numpy.array(grid, dtype=numpy.int)
    grid.shape = (20, 20)
    ...
    for i in range(16):
        for j in range(16):
            slice = []
            for k in range(4):
                slice.append(grid[i+k, j+k])
            slice = numpy.array(slice)
            prod = slice.prod()
            if prod > max:
                max = prod
                imax = i
    ...
    return [max, imax, jmax, dir, nums]

#------------------------------------------------------------------------------#

def f2(grid_string):

    lgrid = [ int(x) for x in grid_string.split() ]
    grid = []
    for i in range(20):
        line = [ int(x) for x in lgrid[i*20:i*20+20] ]
        grid.append(line)
